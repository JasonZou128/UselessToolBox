  ...
  methods: {
    ...
    async onSearch() {
      ...
      this.setData({
        ...
        canBatchSet: res.data.length > 1
      });
    },
    onBatchSetPlan() {
      wx.navigateTo({
        url: `/pages/repairman/inspection-detail/index?batch=true&assetIds=${encodeURIComponent(JSON.stringify(this.data.batchAssetIds))}`
      });
    },
    onDeviceTap(e) {
      const assetId = e.currentTarget.dataset.assetid;
      wx.navigateTo({ url: `/pages/repairman/inspection-detail/index?assetId=${assetId}` });
    }
  }
}); 